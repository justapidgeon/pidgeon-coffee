...
export async function sendOrderNotification(orderData) {
  ...
  const payload = {
    embeds: [{
      title: `☕ New Pidgeon Coffee Order for ${orderData.orderName || "Anonymous"}!`,
      color: 0xc8956c, // --accent color in decimal
      fields: [
        { name: "Order Name / Customer", value: orderData.orderName || "Anonymous", inline: true },
        { name: "Total Amount", value: `$${orderData.total}`, inline: true },
        { name: "Status", value: "Payment Pending (Venmo Redirected)", inline: true },
        { name: "Order Summary", value: itemsList || "No items?", inline: false }
      ],
